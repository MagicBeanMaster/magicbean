function Location() {
	this.items	= {
	'0':{1:'北京市',22:'天津市',44:'上海市',66:'重庆市',108:'河北省',406:'山西省',622:'内蒙古',804:'辽宁省',945:'吉林省',1036:'黑龙江省',1226:'江苏省',1371:'浙江省',1500:'安徽省',1679:'福建省',1812:'江西省',1992:'山东省',2197:'河南省',2456:'湖北省',2613:'湖南省',2822:'广东省',3015:'广西省',3201:'海南省',3235:'四川省',3561:'贵州省',3728:'云南省',3983:'西藏',4136:'陕西省',4334:'甘肃省',4499:'青海省',4588:'宁夏',4624:'新疆',4802:'香港',4822:'澳门',4825:'台湾'},
	'0,1':{2:'北京市'},
	...
	'0,2197,2198':{2199:'中原区',2200:'二七区',2201:'管城回族区',2202:'金水区',2203:'上街区',2204:'惠济区',2205:'新郑市',2206:'登封市',2207:'新密市',2208:'巩义市',2209:'荥阳市',2210:'中牟县',2211:'城关镇',2212:'郑东新区'},
	...
	'0,3235,3236':{3237:'青羊区',3238:'锦江区',3239:'金牛区',3240:'武侯区',3241:'成华区',3242:'龙泉驿区',3243:'青白江区',3244:'新都区',3245:'温江区',3246:'都江堰市',3247:'彭州市',3248:'邛崃市',3249:'崇州市',3250:'金堂县',3251:'赵镇',3252:'双流县',3253:'郫县',3254:'郫筒镇',3255:'大邑县',3256:'晋原镇',3257:'蒲江县',3258:'鹤山镇',3259:'新津县',3260:'五津镇',6000:'高新区'},
...
//根据id获取城市
Location.prototype.filldetail	= function(tr_id,loc_id , selected_id) {
	var json	= this.find(loc_id);
	//alert(json.selected_id);
	if (json) {
		$.each(json , function(k , v) {
			if (k == selected_id) {
				document.getElementById(tr_id).innerHTML=v;
				//alert(v);
				//return v;
			}
		});
...
//根据id获取城市 --用于前后台报备单
Location.prototype.fillreport= function(span_id,loc_id , selected_id) {
	var json	= this.find(loc_id);
	if (json) {
		$.each(json , function(k , v) {
			if (k == selected_id) {				
...
function showLocation(province , city , town) {
	...
	var title	= ['请选择省' , '请选择市' , '请选择区'];
	$.each(title , function(k , v) {
		title[k]	= '<option value="0">'+v+'</option>';
	});
	$('#reportlist_tbFrontCustomer_province').append(title[0]);
	$('#reportlist_tbFrontCustomer_city').append(title[1]);
	$('#loc_town').append(title[2]);
	$('#reportlist_tbFrontCustomer_province').change(function() {
		$('#reportlist_tbFrontCustomer_city').empty();
		$('#reportlist_tbFrontCustomer_city').append(title[1]);
		loc.fillOption('reportlist_tbFrontCustomer_city' , '0,'+$('#reportlist_tbFrontCustomer_province').val());
		$('#loc_town').empty();
		$('#loc_town').append(title[2]);
		
		/*$('#reportlist_tbFrontCustomer_city').selectmenu("refresh"); //2015年11月10日10:45:22 注释，原因报错， hujiang
		$('input[@name=location_id]').val($(this).val()); */
	});
	$('#reportlist_tbFrontCustomer_city').change(function() {
		$('#loc_town').empty();
		$('#loc_town').append(title[2]);
		loc.fillOption('loc_town' , '0,' + $('#reportlist_tbFrontCustomer_province').val() + ',' + $('#reportlist_tbFrontCustomer_city').val());
		/*$('input[@name=location_id]').val($(this).val());*/    //2015年12月11日18:09:58 junjun
	});
	$('#loc_town').change(function() {
		loc.fillOption('town' , '0,' + $('#reportlist_tbFrontCustomer_province').val() + ',' + $('#reportlist_tbFrontCustomer_city').val());   //2015年12月11日18:09:58 junjun
		/*$('input[@name=location_id]').val($(this).val());*/      //2015年12月11日18:09:58 junjun
	});
	if (province) {
		loc.fillOption('reportlist_tbFrontCustomer_province' , '0' , province);
		if (city) {
			loc.fillOption('reportlist_tbFrontCustomer_city' , '0,'+province , city);
			if (town) {
				loc.fillOption('loc_town' , '0,'+province+','+city , town);
			}
		}
	} else {
		loc.fillOption('reportlist_tbFrontCustomer_province' , '0');
	}
}
...
function showLocation2(province , city , town) {
	...
	$('#company_reportlist_tbFrontCustomer_province').append(title[0]);
	$('#company_reportlist_tbFrontCustomer_city').append(title[1]);
	$('#company_loc_town').append(title[2]);
	$('#company_reportlist_tbFrontCustomer_province').change(function() {
		$('#company_reportlist_tbFrontCustomer_city').empty();
		$('#company_reportlist_tbFrontCustomer_city').append(title[1]);
		loc.fillOption('company_reportlist_tbFrontCustomer_city' , '0,'+$('#company_reportlist_tbFrontCustomer_province').val());
		$('#company_loc_town').empty();
		$('#company_loc_town').append(title[2]);
		
		/*$('#reportlist_tbFrontCustomer_city').selectmenu("refresh"); //2015年11月10日10:45:22 注释，原因报错， hujiang
		$('input[@name=location_id]').val($(this).val()); */
	});
	$('#company_reportlist_tbFrontCustomer_city').change(function() {
		$('#company_loc_town').empty();
		$('#company_loc_town').append(title[2]);
		loc.fillOption('company_loc_town' , '0,' + $('#company_reportlist_tbFrontCustomer_province').val() + ',' + $('#company_reportlist_tbFrontCustomer_city').val());
		/*$('input[@name=location_id]').val($(this).val());*/    //2015年12月11日18:09:58 junjun
	});
	$('#company_loc_town').change(function() {
		loc.fillOption('town' , '0,' + $('#company_reportlist_tbFrontCustomer_province').val() + ',' + $('#company_reportlist_tbFrontCustomer_city').val());   //2015年12月11日18:09:58 junjun
		/*$('input[@name=location_id]').val($(this).val());*/      //2015年12月11日18:09:58 junjun
	});
	if (province) {
		loc.fillOption('company_reportlist_tbFrontCustomer_province' , '0' , province);
		if (city) {
			loc.fillOption('company_reportlist_tbFrontCustomer_city' , '0,'+province , city);
			if (town) {
				loc.fillOption('company_loc_town' , '0,'+province+','+city , town);
			}
		}
	} else {
		loc.fillOption('company_reportlist_tbFrontCustomer_province' , '0');
	}
}
